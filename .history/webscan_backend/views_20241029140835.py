...
def finger_print(request):
    ...
    if url:
        ...
        # 整合所有结果
        result = {
            'domain': base_info.get('data', {}).get('domain', 'error'),
            'source_ip': base_info.get('data', {}).get('ip', ['error'])[0] if base_info.get('data', {}).get('ip') else 'error',
            'script_lang': base_info.get('data', {}).get('language', 'error'),
            'server_os': base_info.get('data', {}).get('os', 'error'),
            'server_middleware': base_info.get('data', {}).get('server', 'error'),
