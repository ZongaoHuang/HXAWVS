...
def finger_print(request):
    ...
    try:
        ...
        thread = Thread(target=run_finger_print_scan, args=(scan, url))
        thread.daemon = True
        thread.start()
        ...
        return error(500, str(e), 'error')
    
def run_finger_print_scan(scan, url):
    """
    执行指纹扫描的异步函数
    """
    try:
        ...
        scan.status = 'error'
        scan.save()
        MYLOGGER.error(f'Finger print scan failed for {url}: {str(e)}')
