...
def finger_print(request):
    ...
    if url:
        ...
        site_weight = get_web_weight(url)
        cdn_info = iscdn(url)
        waf_info = getwaf(url)
        cms_info = getwhatcms(url)
        result = {
                'source_ip': getdomainip(getdomain(url)),
                'script_lang': base_info.get('language', '未知'),
                'server_os': base_info.get('os', '未知'),
                'server_middleware': base_info.get('server', '未知'),
                'site_weight': site_weight,
                'cdn_isexsit': cdn_info,
                'waf': waf_info,
                'cms_finger': cms_info
        }
